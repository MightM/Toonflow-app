...
3. 按下方「提取规则」逐资产识别衍生：**角色只提取变身状态，场景提取时间变体与状态变体，道具只提取使用状态**
...
- **严格遵守提取范围**：角色仅限变身状态（服装 / 变身特效 / 变形）、场景仅限时间变体与状态变体、道具仅限使用状态；超出范围的状态不得写入
...
> **角色基准态**：角色父资产默认即为该角色对应身份的基础着装，由 `art_character.md` 生成一张人物多视图（主大头照 + 正/侧/背全身）。变身/换装类衍生按对应风格的 `art_character_derivative.md` 以"参考父角色多视图 + 只替换服化"的方式生成新的多视图。
> **场景基准态**：场景父资产默认即为该场景的基础时段视图（由 `art_scene.md` 生成）。时间变体、状态变体类衍生按对应风格的 `art_scene_derivative.md` 以"参考主视图 + 目标时段/状态"方式落地。
> **道具基准态**：道具父资产默认即为该道具的常规完好状态（由 `art_prop.md` 生成）。使用状态类衍生按对应风格的 `art_prop_derivative.md` 以"参考父道具当前图（三视图）+ 目标状态"方式落地。
...
| 场景 | 是 | ①**时间变体**；②**状态变体** | 日景→夜景、黄昏版；火灾后焦黑、被砸毁、婚礼布置后 |
| 道具 | 是 | **仅使用状态** | 完整→碎裂、合上→打开、熄灭→点燃、崭新→染血 |
...
- **场景**：提取两类——①「时间变体」：同一场景在不同时段下的整体光照/色调/氛围变化（如日景→夜景、黄昏、清晨）；②「状态变体」：场景本身发生的持续性改变（如被毁、焚烧后、重新布置）。同一场景可有多个变体，各自独立；机位角度、临时天气、人物活动带来的变化**不提取**
- **道具**：只提取「使用状态」——道具本身外观发生的持续性改变（如碎裂、打开、点燃、染血、磨损）。手持方式、摆放位置、镜头角度**不提取**
- 角色变身/变形类特征变体、场景状态变体、道具使用状态必须同时满足：**稳定、可复用、资产级**。仅在多个镜头/场次中持续成立，且会改变资产整体识别外观时才创建
